...
function isItem(value: unknown): value is Item {
  ...
  return (
    ...
    !Number.isNaN(Date.parse(v.createdAt as string))
  )
}

/** 宽松解析导入内容：补齐缺失字段，无法识别的条目直接丢弃 */
function normalizeImported(raw: unknown): Item[] {
  if (!Array.isArray(raw)) throw new Error('文件内容不是收藏列表（应为 JSON 数组）')
  const items: Item[] = []
  for (const entry of raw) {
    if (typeof entry !== 'object' || entry === null) continue
    const e = entry as Record<string, unknown>
    if (typeof e.title !== 'string' || !e.title.trim() || !isCategory(e.category)) continue
    items.push({
  ...
  if (items.length === 0) throw new Error('没有找到可导入的有效收藏')
...
export function CollectionProvider({ children }: { children: ReactNode }) {
  const [items, setItems] = useState<Item[]>(loadInitial)
  ...
  const importItems = useCallback((jsonText: string) => {
  ...
  const accessionOf = useCallback((id: string) => accessionMap.get(id), [accessionMap])

  const value = useMemo<CollectionContextValue>(
    () => ({ items, accessionOf, addItem, updateItem, removeItem, importItems, resetToSeed, exportItems }),
    [items, accessionOf, addItem, updateItem, removeItem, importItems, resetToSeed, exportItems],
  )
...
export function useCollection(): CollectionContextValue {
  ...
  if (!ctx) throw new Error('useCollection 必须在 CollectionProvider 内使用')
  return ctx
}
